<?php
class Clientlogos extends Trongate {
    ...
    function create() {
        ...
        if (is_numeric($update_id)) {
            $data['headline'] = 'Update Clientlogo Record';
            $data['cancel_url'] = BASE_URL.'clientlogos/show/'.$update_id;
        } else {
            $data['headline'] = 'Create New Clientlogo Record';
            $data['cancel_url'] = BASE_URL.'clientlogos/manage';
        }

        $data['form_location'] = BASE_URL.'clientlogos/submit/'.$update_id;
    ...
    function manage() {
        ...
        if (segment(4) !== '') {
            ...
            $sql = 'select * from clientlogos
            WHERE client_name LIKE :client_name
            ORDER BY id desc';
            $all_rows = $this->model->query_bind($sql, $params, 'object');
        } else {
            $data['headline'] = 'Manage Clientlogos';
        ...
        $pagination_data['pagination_root'] = 'clientlogos/manage';
        $pagination_data['record_name_plural'] = 'clientlogos';
    ...
    function show() {
        ...
        if ($data == false) {
            ...
            $data['headline'] = 'Clientlogo Information';
            $data['view_file'] = 'show';
            $this->template('bootstrappy', $data);
        }
    }

    function homepage_clientlogos(){
        $clientlogos = $this->model->get('id desc');
        return $clientlogos;
    }
        ...
        if ($submit == 'Submit') {
            ...
            if ($result == true) {
                ...
                if (is_numeric($update_id)) {
                    ...
                    $this->model->update($update_id, $data, 'clientlogos');
                    $flash_msg = 'The record was successfully updated';
                } else {
                    //insert the new record
                    $update_id = $this->model->insert($data, 'clientlogos');
                    $flash_msg = 'The record was successfully created';
                }

                set_flashdata($flash_msg);
                redirect('clientlogos/show/'.$update_id);
    ...
    function submit_delete() {
        ...
        if (($submit == 'Yes - Delete Now') && (is_numeric($params['update_id']))) {
            ...
            $params['module'] = 'clientlogos';
            $this->model->query_bind($sql, $params);

            //delete the record
            $this->model->delete($params['update_id'], 'clientlogos');
    ...
    function _get_data_from_db($update_id) {
        $record_obj = $this->model->get_where($update_id, 'clientlogos');
    ...
    function _init_picture_settings() { 
        ...
        $picture_settings['destination'] = 'clientlogos_pics';
        $picture_settings['target_column_name'] = 'picture';
        $picture_settings['thumbnail_dir'] = 'clientlogos_pics_thumbnails';
